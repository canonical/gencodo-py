...
class BaseCommand:
    ...
    It may also set ``examples`` and ``related_commands`` for documentation generation.
    ...
    """Usage examples for documentation generation.

    Each entry is a tuple of (description, command_string). Subclasses declare
    this as a class-level attribute. Defaults to empty list.
    """

    related_commands: list[str] | None = None
    """Names of related commands for documentation cross-references.

    When None, the documentation generator infers related commands from other
    non-hidden commands in the same CommandGroup. Defaults to None.
    """
